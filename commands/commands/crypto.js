...
module.exports = async (command, args, msg, user, db) => {
    ...
    const PRICE_INTERVAL = 15 * 60 * 1000; // Harga: 15 Menit
    const STOCK_INTERVAL = 3 * 60 * 1000;  // Stok: 3 Menit
    const TAX_SELL = 0.02; 
    const MARGIN_INTEREST = 0.05; 

    // ==========================================
    // A. TIME TRAVEL: UPDATE HARGA (Catch-Up)
    // ==========================================
    let missedPriceSteps = Math.floor((now - marketData.lastPriceUpdate) / PRICE_INTERVAL);
    
    if (missedPriceSteps > 0) {
        const steps = Math.min(missedPriceSteps, 50); // Max 50 cycles
        ...
        for (let i = 0; i < steps; i++) {
            ...
            for (let k in marketData.prices) {
                ...
                // Smart Floor Price (Harga Minimal)
                let floorPrice = MIN_PRICES[k] || 1;
                let newPrice = Math.max(floorPrice, Math.floor(rawPrice));

                // Auto Recovery (Bandar Pump)
                if (newPrice <= floorPrice * 1.1) {
                    if (Math.random() < 0.7) newPrice += Math.floor(newPrice * 0.2) + 5;
                }
        ...
        marketData.lastPriceUpdate += (missedPriceSteps * PRICE_INTERVAL);
        saveDB(db);
    }

    // ==========================================
    // B. TIME TRAVEL: UPDATE STOK (Catch-Up)
    // ==========================================
    let missedStockSteps = Math.floor((now - marketData.lastStockUpdate) / STOCK_INTERVAL);

    if (missedStockSteps > 0) {
        const steps = Math.min(missedStockSteps, 100); // Max 100 cycles
        ...
        for (let i = 0; i < steps; i++) {
            for (let k in marketData.prices) {
                ...
                // Bandar Restock (Anti-Langka)
                if (marketData.stocks[k] < 20) {
                    actualChange += Math.floor(Math.random() * 50) + 30;
                }
        ...
        marketData.lastStockUpdate += (missedStockSteps * STOCK_INTERVAL);
        saveDB(db);
    }
    ...
    if (command === 'market') {
        ...
        let sSec = Math.floor((stockTime % 60000) / 1000);
    ...
    if (command === 'buycrypto') {
        ...
        user.balance -= total; marketData.stocks[koin] -= jml;
    ...
    if (command === 'margin' || command === 'ngutang') {
        ...
        if (args[1]?.toLowerCase() === 'all') {
            ...
            jml = Math.floor(jml * 10000) / 10000; 
        } else {
            jml = parseFloat(args[1]?.replace(',', '.'));
        }
        if (isNaN(jml) || jml <= 0) return msg.reply("❌ Jumlah salah.");
        ...
        user.debt += biaya; user.crypto[koin] = (user.crypto[koin] || 0) + jml; saveDB(db);
    ...
    if (command === 'paydebt' || command === 'bayar') {
        ...
        let bayar = 0;
        if (args[0]?.toLowerCase() === 'all') { bayar = totalHutang; } 
        else { bayar = parseInt(args[0]?.replace(/[.,]/g, '') || '0'); }
        if (isNaN(bayar) || bayar <= 0) return msg.reply("❌ Nominal salah.");
        if (user.balance < bayar) return msg.reply(`❌ Saldo kurang! Butuh 💰${fmt(bayar)}`);
        if (bayar > totalHutang) bayar = totalHutang;
        user.balance -= bayar; user.debt -= bayar; saveDB(db);
